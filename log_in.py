...
class AuthenticationSystem:
    ...
    def __init__(self):
        ...
        self.users = pd.DataFrame(columns=["user_id", "username", "password", "failed_attempts_count", "is_locked"])
    ...
    def login(self, username, password):
        ...
        user = User(user_row['user_id'].values[0], user_row['username'].values[0], user_row['password'].values[0], 
                    ...
                    user_row['failed_attempts_count'].values[0], user_row['is_locked'].values[0])
    ...
    def update_user(self, user):
        ...
        self.users.loc[self.users['username'] == user.username, 'failed_attempts_count'] = user.failed_attempts
...
auth_system.login("neena", "password321") 
auth_system.login("neena", "password321") # lock account test
auth_system.login("Neena", "password123")  # case sensitive test
auth_system.login("neena", "password123") # successful login test
auth_system.login("neena", "password321") # reset failed attempts test
auth_system.login("helios", "password321")  # failed login test
auth_system.login("helios", "mysecurepassword") # successful login test